...
import cl.duoc.app.model.data.config.Converters;
...
public final class PurchaseDao_Impl implements PurchaseDao {
  ...
  private final SharedSQLiteStatement __preparedStmtOfDeleteAllUserPurchases;
  ...
  public PurchaseDao_Impl(@NonNull final RoomDatabase __db) {
    ...
    this.__insertionAdapterOfPurchaseEntity = new EntityInsertionAdapter<PurchaseEntity>(__db) {
      ...
      protected String createQuery() {
        return "INSERT OR REPLACE INTO `purchases` (`id`,`userId`,`total`,`shippingAddress`,`paymentMethod`,`status`,`createdAt`) VALUES (nullif(?, 0),?,?,?,?,?,?)";
      }
    ...
    this.__updateAdapterOfPurchaseEntity = new EntityDeletionOrUpdateAdapter<PurchaseEntity>(__db) {
      ...
      @NonNull
      protected String createQuery() {
        return "UPDATE OR ABORT `purchases` SET `id` = ?,`userId` = ?,`total` = ?,`shippingAddress` = ?,`paymentMethod` = ?,`status` = ?,`createdAt` = ? WHERE `id` = ?";
      }
    ...
    this.__preparedStmtOfDeleteAllUserPurchases = new SharedSQLiteStatement(__db) {
      @Override
      @NonNull
  ...
  @Override
  public Object deleteAllUserPurchases(final int userId,
      final Continuation<? super Unit> $completion) {
    return CoroutinesRoom.execute(__db, true, new Callable<Unit>() {
      @Override
      @NonNull
      public Unit call() throws Exception {
        final SupportSQLiteStatement _stmt = __preparedStmtOfDeleteAllUserPurchases.acquire();
        int _argIndex = 1;
        ...
        try {
          ...
          __preparedStmtOfDeleteAllUserPurchases.release(_stmt);
        }
      }
    }, $completion);
  }
  ...
  public Flow<List<PurchaseEntity>> getUserPurchases(final int userId) {
    final String _sql = "SELECT * FROM purchases WHERE userId = ? ORDER BY createdAt DESC";
      ...
      public List<PurchaseEntity> call() throws Exception {
        ...
        try {
          ...
          while (_cursor.moveToNext()) {
            ...
            _item = new PurchaseEntity(_tmpId,_tmpUserId,_tmpTotal,_tmpShippingAddress,_tmpPaymentMethod,_tmpStatus,_tmpCreatedAt);
            _result.add(_item);
          }
          return _result;
        } finally {
  ...
  @Override
  public Flow<PurchaseEntity> getPurchaseById(final int purchaseId) {
      ...
      public PurchaseEntity call() throws Exception {
        ...
        try {
          ...
          if (_cursor.moveToFirst()) {
            ...
            _result = new PurchaseEntity(_tmpId,_tmpUserId,_tmpTotal,_tmpShippingAddress,_tmpPaymentMethod,_tmpStatus,_tmpCreatedAt);
          } else {
            _result = null;
          }
          return _result;
